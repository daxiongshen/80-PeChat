...
// 
// So we use a primitive logging macro around PETERLog.
// We maintain the NS prefix on the macros to be explicit about the fact that we're using PETERLog.

#define DD_DEBUG NO

#define PETERLogDebug(frmt, ...) do{ if(DD_DEBUG) PETERLog((frmt), ##__VA_ARGS__); } while(0)
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		PETERLogDebug(@"DDLog: Using grand central dispatch");
		...
		PETERLogDebug(@"DDLog: numProcessors = %u", numProcessors);
...
+ (void)lt_removeLogger:(id <DDLogger>)logger
{
	...
	if (loggerNode == nil)
	{
		PETERLogDebug(@"DDLog: Request to remove logger which wasn't added");
		return;
	}
